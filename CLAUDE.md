...
There is no build system, package manager, or test suite — just the scripts, README.md (short, user-facing) and MANUAL.md (technical reference). There *are* lint gates: a GitHub Actions CI pipeline (osacompile syntax check of both AppleScripts on macOS, shellcheck of `forti-debug.scpt` on Ubuntu) and pre-commit hooks (whitespace/EOF/yaml checks plus the same shellcheck).
